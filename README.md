...
| **L2: 要件** | `SPEC-XXX` | `REQ-XXX.md` | **具体的要件**: 要求を分解した、テスト可能な詳細仕様。REQ に紐付く。 |
| **L3: 機能** | `FUNC-XXX` | `REQ-XXX.md` | **論理機能**: 要件を実現するための具体的な機能定義。SPEC に紐付く。 |
| **L4: 構造** | `COMP-XXX` | `architecture.md` | **コンポーネント**: 機能を実装レベルのモジュールに分割。FUNC に紐付く。 |
...
- **Single Source of Truth**: `requirements.md` と `architecture.md` を唯一の正典として、設計資産を一元管理。
...
- **specification-detail-management**: 抽象的な要求（REQ）を、具体的でテスト可能な詳細要件（SPEC/FUNC/DATA）にパッケージ内で詳細化。
- **strategy-decision-maker**: 要求に対し、詳細分析に進むかプロトタイプを作成するかの実行戦略を決定。
- **question-management**: プロジェクト内の質疑応答や意思決定事項を Q&A 形式で記録。

### 構造設計・UX設計
- **architecture-design-expert**: 物理的な配置（COMP/UNIT/DEP）を `architecture.md` に定義・管理。
- **uiflow-expert**: ユーザー体験（UX）を「見る・する・遷移する」の連鎖として設計し、Mermaid 形式で可視化。

### 実装・プロトタイピング
- **design-implementation-expert**: TDD による実装を担当。各要求パッケージを唯一の論理的な真実（SSOT）として扱います。
...
### 補助・メタスキル
- **reverse-document-expert**: 既存コードやプロトタイプから設計ドキュメントを逆生成し、実装の妥当性を評価。
